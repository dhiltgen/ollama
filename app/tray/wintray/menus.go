...
func (t *winTray) initMenus() error {
	...
	if os.Getenv("OLLAMA_HOST") != "" {
		slog.Debug("OLLAMA_HOST set in environment, disabling menu toggle")
	...
	if os.Getenv("OLLAMA_ORIGINS") != "" {
		slog.Debug("OLLAMA_ORIGINS set in environment, disabling menu toggle")
...
func (t *winTray) UpdateAvailable(ver string) error {
	if !t.updateNotified {
		...
		if err := t.addOrUpdateMenuItem(updateMenuID, 0, commontray.UpdateMenuTitle, false, false); err != nil {
			return fmt.Errorf("unable to create menu entries %w", err)
		}
