...
void dequantize_mxfp4(device const block_mxfp4 * xb, short il, thread type4x4 & reg) {
    float4x4 reg_f;
    const float scale = (as_type<float>(((uint32_t)xb->d) << 23));
    float mxfp4_table[] = MXFP4_VALS;
    ...
    for (int i = 0; i < 8; i++) {
        if (isnan(scale)) {
            ...
            reg_f[i/2][2*(i%2) + 0] = scale * mxfp4_table[(xb->qs[il*8 + i] & 0xf)];
            reg_f[i/2][2*(i%2) + 1] = scale * mxfp4_table[(xb->qs[il*8 + i] & 0xf0) >> 4];
        }
    }
    reg = (type4x4) reg_f;
}
...
#pragma unroll
        for (short row = 0; row < nr0; row++) {
            ...
            float scale = as_type<float>(((uint32_t)(ax[row] + ib)->d) << 23);
            // il = 0 or 16
            ...
            for (int i = 0; i < 8; ++i) {
                if (isnan(scale)) {
                    ...
                    sumf[row] += yb[i*2] * mxfp4_table[(qs[i] & 0xf)] * scale
                        + yb[i*2+1] * mxfp4_table[(qs[i] & 0xf0) >> 4] * scale;
                }
            }
        }
